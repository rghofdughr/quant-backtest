...
TRADING_DAYS = 252
DESCRIPTION = "Cross-sectional Sharpe ranking: long top decile by trailing 12m Sharpe (ann_ret/ann_vol). Requires 200d min history."
...
def run(config):
    ...
    for rd in reb_dates:
        ...
        for c in valid_cols:
            ...
            if vol < 1e-9:
                continue
            sr = float(r.mean() / vol * np.sqrt(TRADING_DAYS))
            if np.isfinite(sr):
                sharpes[c] = sr
