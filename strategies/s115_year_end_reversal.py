"""
s115 — Year-End Tax-Loss Reversal (R1000)
Signal: In November each year, buy R1000 stocks with worst 3-month return
(tax-loss selling candidates). Hold through end of January (reversal).
Based on: Ritter (1988) January Effect / tax-loss reversal.
"""
import sys, os
...
DESCRIPTION = "Year-end tax-loss reversal: buy worst 3m R1000 in November, hold through January"
...
def run(config):
    ...
    # Signal: enter on first trading day of November, hold through last trading day of January
    for yr in range(idx.year.min() + 1, idx.year.max() + 1):
        # Entry: first November trading day of year yr
        nov_days = idx[(idx.year == yr) & (idx.month == 11)]
        if len(nov_days) == 0:
            continue
        entry_date = nov_days[0]
        ...
        mom = mom_df.loc[entry_date, valid].dropna()
        ...
        weight_schedule[entry_date] = w

        # Exit: last January trading day of year yr+1
        jan_days = idx[(idx.year == yr + 1) & (idx.month == 1)]
        if len(jan_days) > 0:
            weight_schedule[jan_days[-1]] = {}  # go to cash
